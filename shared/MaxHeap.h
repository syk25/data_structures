...
template <typename T> class MaxHeap {
  public:
    ...
    void Push(const T &item) {
        ...
        int parent_idx = current / 2;

        while (current != 1 && (heap_[parent_idx] < item) /* && TODO */) // 부모 위치의 값이 추가하려는 값보다 작다면
        {
            // 부모 위치의 값을 자식 위치로 복사해서 내린다.
            // REVIEW
            heap_[current] = heap_[parent_idx];
    ...
    void Pop() {
        ...
        while (child <= size_) {
            ...
            if (child == size_) {
                ...
                higher = heap_[left] >= heap_[right] ? left : right;
            }

            // 현재(current) 값이 더 큰 자식의 값 이상이면 더이상 적절한 위치를 찾을 필요가 없기 때문에 루프 중단
            // REVIEW
            if (last_item >= heap_[higher]) {
                break;
            } else {
