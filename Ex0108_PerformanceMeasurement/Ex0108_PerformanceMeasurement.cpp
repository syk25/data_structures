    ...
    for (int n = 1; n <= kNumTest; n += int(pow(2, 10))) {
        ...
        for (int x = 0; x < n + 1; x++) // 못 찾는 경우를 하나 마지막에 추가
        {
            ...
            // Sequential Search
            {
                ...
                // result_seq = SequentialSearch(arr, n, n + rand() % 1000); //
                // TEST 2 <- 항상 못찾도록
                auto stop = high_resolution_clock::now();
                auto duration =
                    double(duration_cast<nanoseconds>(stop - start).count()) /
                    1000.0;
                seq_sum += duration;
            ...
            // Binary Search
            {
                ...
                // result_bi = BinarySearch(arr, n, n + rand() % 1000); // TEST
                // 2<- 항상 못찾도록
                auto stop = high_resolution_clock::now();
                auto duration =
                    double(duration_cast<nanoseconds>(stop - start).count()) /
        ...
        cout << n << ", " << seq_max << ", " << bi_max << ", " << seq_sum
             << ", " << bi_sum << ", " << seq_min << ", " << bi_min << endl;
        ofile << n << ", " << seq_max << ", " << bi_max << ", " << seq_sum
              << ", " << bi_sum << ", " << seq_min << ", " << bi_min << endl;
