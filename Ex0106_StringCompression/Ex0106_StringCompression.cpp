...
void print_letter(char *arr, int n, int i) {
    int count = Count(arr, n, char(i + 'a'));
    if (count != 0) {
        cout << char(i + 'a') << count;
    }
}
...
int main() {
    ...
    for (int i = 0; i < 26; i++) {
        ...
        // NOTE: 표를 사용하는 경우
        table[i] = Count(arr, n, char(i + 'a'));

        // NOTE: 직접 출력하는 방법
        // print_letter(arr, n, i);
    }
    // Print(table, sizeof(table) / sizeof(int));
