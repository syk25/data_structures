  ...
  public:
    ...
    void NewTerm(float coef, int exp) {
        ...
        if (!first_) {
            ...
            Node *current = first_;
            while (current) {
                if (current->item.exp == term->item.exp) {
                    current->item.coef += term->item.coef;
                    delete term;
                    return;
                } else if (current->item.exp > term->item.exp) {
                    term->next = current;
                    prev->next = term;
                    return;
                } else {
    ...
    float Eval(float x) {
        ...
        Node *current = first_;
        while (current) {
            temp += current->item.coef * std::powf(x, current->item.exp);
            current = current->next;
        }
    ...
    LinkedPolynomial Add(const LinkedPolynomial &poly) {
        ...
        while (i && j) {
            if (i->item.exp < j->item.exp) {
                ...
                Term term = {i->item.coef + j->item.coef, i->item.exp};
                temp.PushBack(term);
