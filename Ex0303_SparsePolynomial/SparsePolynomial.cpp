...
float SparsePolynomial::Eval(float x) {
    ...
    for (int i = 0; i < num_terms_; i++) {
        temp += this->terms_[i].coef * std::powf(x, this->terms_[i].exp);
    }
...
SparsePolynomial SparsePolynomial::Add(const SparsePolynomial &poly) {
    ...
    while (i < this->num_terms_ && j < poly.num_terms_) {
        i_exp = this->terms_[i].exp;
        j_exp = poly.terms_[j].exp;
        i_coef = this->terms_[i].coef;
        j_coef = poly.terms_[j].coef;
        if (i_exp < j_exp) {
            temp.NewTerm(i_coef, i_exp);
            i++;
        } else if (i_exp > j_exp) {
            temp.NewTerm(j_coef, j_exp);
            j++;
        } else {
            temp.NewTerm(i_coef + j_coef, i_exp);
            i++;
            j++;
        }
    }
